...
{
	// sendPost에서 보낼 Packet이 없는 경우를 걸러냈는데도 이 조건이 또다시 발생하는 경우는 error
	...
	// 전송 flag를 다시 미전송 상태로 되돌리기
	InterlockedExchange8((char*)&pSession->sendFlag, false);

	// 1회 send 하는 과정에서 sendQ에 쌓여버린 나머지 데이터 모두 send
	if (pSession->sendQ.GetSize() > 0)
	{
...
bool NetServer::RecvPost(stSESSION* pSession)
{
	...
	if (recvRet == SOCKET_ERROR)
	{
		...
		else
		{
			...
			// Pending 걸렸는데, 이 시점이 disconnect 된 후라면 이 때 남아있던 비동기 io를 정리해줘야함
			if (pSession->isDisconnected)
			{
